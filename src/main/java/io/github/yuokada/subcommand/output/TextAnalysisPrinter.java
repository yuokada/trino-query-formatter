...
public final class TextAnalysisPrinter implements AnalysisPrinter {
  ...
  private void printFull(QueryAnalysisResult r, Integer queryId) {
    ...
    String flags = String.format(
        ...
        Boolean.toString(r.isUsesSelectStar()),
        String.valueOf(r.getHasLimit()),
        String.valueOf(r.getHasWhereOnDelete())
    );
    emitter.emit(flags);
    if (!r.getFunctionsScalar().isEmpty() || !r.getFunctionsAggregate().isEmpty() || !r.getFunctionsWindow().isEmpty()) {
    ...
    if (!r.getWriteTargets().isEmpty()) {
      emitter.emit(String.format("WriteTargets: [%s]", String.join(",", r.getWriteTargets())));
    }
